...
function Navbar() {
  ...
  // Fechar os menus ao clicar fora ou ao pressionar Escape
  useEffect(() => {
  ...
  const handleNavegacao = (to: string) => {
    setIsMenuOpen(false);
    setIsMobileMenuOpen(false);
    navigate(to);
  };
  ...
  const handleAreaCliente = () => {
    ...
    if (!isAutenticado) {
      ...
      ToastAlerta('Acesso negado: Seu perfil é de Corretor.', 'erro');
    }
  };
  ...
  return (
    <nav
      ref={dropdownRef}
      className="fixed top-6 left-1/2 -translate-x-1/2 w-[92%] max-w-6xl bg-white/90 backdrop-blur-md shadow-xl border border-zinc-200/60 rounded-3xl sm:rounded-full px-4 sm:px-6 py-3 z-100 transition-all duration-300"
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {/* Botão Hambúrguer Mobile (Links das páginas) */}
          <button
            type="button"
            onClick={toggleMobileMenu}
            aria-label="Abrir menu de navegação"
            className="lg:hidden flex items-center justify-center w-10 h-10 rounded-full bg-zinc-100 hover:bg-zinc-200 text-zinc-700 transition-colors cursor-pointer focus:outline-none"
          >
            {isMobileMenuOpen ? (
              <svg
              ...
              >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider text-red-600 bg-red-50 px-1.5 py-0.5 rounded border border-red-100">
                          {(usuario.perfil || localStorage.getItem('perfil')) === 'ROLE_CORRETOR'
                            ? 'Corretor'
                            : 'Segurado'}
                        </span>
                      </div>
                ...
                {usuario.token ? (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className="block text-[11px] text-zinc-400 font-normal">Acesse sua conta</span>
                    </div>
                  </button>
                )}
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="block text-[11px] text-zinc-400 font-normal">Minhas apólices e coberturas</span>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="block text-[11px] text-zinc-400 font-normal">Gestão de apólices</span>
                  </div>
                </button>
                ...
                {usuario.token && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className="block text-[11px] text-zinc-400 font-normal">Encerrar sessão</span>
                    </div>
                  </button>
                )}
              </div>
      ...
      {/* MENU MOBILE EXPANSÍVEL (Abre diretamente embaixo da Navbar) */}
      {isMobileMenuOpen && (
        <div className="lg:hidden mt-3 pt-3 border-t border-zinc-200/60 animate-in fade-in slide-in-from-top-2 duration-200 space-y-1">
          {links.map((link) => (
            <button
              key={link.text}
              type="button"
              onClick={() => handleNavegacao(link.to)}
              className={`w-full text-left px-4 py-2.5 text-sm font-medium rounded-2xl transition-all cursor-pointer flex items-center justify-between ${
                location.pathname === link.to
            ...
            >
              ...
              <svg
                ...
                strokeWidth={2}
                stroke="currentColor"
                className={`w-4 h-4 ${location.pathname === link.to ? 'text-red-600' : 'text-zinc-400'}`}
              >
