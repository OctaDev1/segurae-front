...
function Navbar() {
  ...
  return (
    ...
    >
      <Link to="/" onClick={() => { setIsMenuOpen(false); setIsMobileMenuOpen(false); }} className="flex items-center shrink-0">
        <img
      ...
      {/* MENU MOBILE EXPANSÍVEL (Links Início, Coberturas, Serviços, Contato) */}
      {isMobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 mt-3 bg-white/95 backdrop-blur-md rounded-3xl shadow-xl shadow-zinc-900/10 border border-zinc-200/70 p-4 z-[110] animate-in fade-in zoom-in-95 duration-150 space-y-1">
          {links.map((link) => (
            <button
              key={link.text}
              type="button"
